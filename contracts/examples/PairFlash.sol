...
import 'depin-dex-v3-core/contracts/interfaces/callback/IDepinDexFlashCallback.sol';
import 'depin-dex-v3-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the DepinDex flash function
contract PairFlash is IDepinDexFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `DepinDexFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IDepinDexPool pool = IDepinDexPool(PoolAddress.computeAddress(factory, poolKey));
